...
async function uploadFile() {
  ...
  const paths = [
    {
      d: "M19.9189 0L24.5667 4.64773L29.2144 9.29545H19.9189V0Z",
      fill: "#92DBFF"
    },
    {
  ...
  async function anal() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const fillerWordsText = Object.entries(fillerWords)
          .map(([word, count]) => `${word}: ${count}번`)
          .join(", ");
...
async function removeFile() {
  ...
  document.getElementById("result_font1").innerText =
    "분석할 발표 영상을 업로드 해주세요";
...
function truncateText(text, maxLength) {
  ...
  if (truncated.length > maxLength) {
    truncated =
      truncated.substr(0, maxLength - 5) + " · · · " + truncated.substr(-5);
  }
  return truncated;
}
